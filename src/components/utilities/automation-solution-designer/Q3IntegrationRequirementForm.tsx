...
import { Q3Answer } from '@/lib/types/process-orchestration';
import { PROCESS_ORCHESTRATION_QUESTIONS } from '@/lib/constants/process-orchestration';

interface Q3IntegrationRequirementFormProps {
  onNext: (answer: Q3Answer) => void;
  onPrevious?: () => void;
  currentAnswer?: Q3Answer;
}

export function Q3IntegrationRequirementForm({
  onNext,
  onPrevious,
  currentAnswer,
}: Q3IntegrationRequirementFormProps) {
  const [selected, setSelected] = React.useState<Q3Answer | undefined>(currentAnswer);
  const question = PROCESS_ORCHESTRATION_QUESTIONS.Q3;

  const handleSelect = (optionId: string) => {
    setSelected(optionId as Q3Answer);
  };
