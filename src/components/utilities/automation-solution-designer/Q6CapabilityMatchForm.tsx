...
import { Q6Answer } from '@/lib/types/process-orchestration';
import { PROCESS_ORCHESTRATION_QUESTIONS } from '@/lib/constants/process-orchestration';

interface Q6CapabilityMatchFormProps {
  onNext: (answer: Q6Answer) => void;
  onPrevious: () => void;
  currentAnswer?: Q6Answer;
}

export function Q6CapabilityMatchForm({
  onNext,
  onPrevious,
  currentAnswer,
}: Q6CapabilityMatchFormProps) {
  const [selected, setSelected] = React.useState<Q6Answer | undefined>(currentAnswer);
  const question = PROCESS_ORCHESTRATION_QUESTIONS.Q6;

  const handleSelect = (optionId: string) => {
    setSelected(optionId as Q6Answer);
  };
