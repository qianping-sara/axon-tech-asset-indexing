...
import { Q1Answer } from '@/lib/types/process-orchestration';
import { PROCESS_ORCHESTRATION_QUESTIONS } from '@/lib/constants/process-orchestration';

interface Q1StrategicIntentFormProps {
  onNext: (answer: Q1Answer) => void;
  currentAnswer?: Q1Answer;
}

export function Q1StrategicIntentForm({
  onNext,
  currentAnswer,
}: Q1StrategicIntentFormProps) {
  const [selected, setSelected] = React.useState<Q1Answer | undefined>(currentAnswer);
  const question = PROCESS_ORCHESTRATION_QUESTIONS.Q1;

  const handleSelect = (optionId: string) => {
    setSelected(optionId as Q1Answer);
  };
