...
import { Q5Answer } from '@/lib/types/process-orchestration';
import { PROCESS_ORCHESTRATION_QUESTIONS } from '@/lib/constants/process-orchestration';

interface Q5LogicComplexityFormProps {
  onNext: (answer: Q5Answer) => void;
  onPrevious: () => void;
  currentAnswer?: Q5Answer;
}

export function Q5LogicComplexityForm({
  onNext,
  onPrevious,
  currentAnswer,
}: Q5LogicComplexityFormProps) {
  const [selected, setSelected] = React.useState<Q5Answer | undefined>(currentAnswer);
  const question = PROCESS_ORCHESTRATION_QUESTIONS.Q5;

  const handleSelect = (optionId: string) => {
    setSelected(optionId as Q5Answer);
  };
